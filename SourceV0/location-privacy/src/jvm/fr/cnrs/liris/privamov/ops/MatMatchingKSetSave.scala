 ...
 * Contributors: Mohamed Maouche  <mohamed.maouchet@liris.cnrs.fr>MatMatchingKSetsSave.scala
 *
...
class MatMatchingKSetsSaveOp extends Operator[MatMatchingKSetsSaveIn, MatMatchingKSetsSaveOut] with SparkleOperator {


  override def execute(in: MatMatchingKSetsSaveIn, ctx: OpContext): MatMatchingKSetsSaveOut = {
    // read the data
    ...
    MatMatchingKSetsSaveOut(matches, klevel, distances,probabilities, rate)
  }

  def reIdentAttack(rdstrain: DataFrame[Trace], rdstest: DataFrame[Trace], dimensions: (Int, Int, Point), in: MatMatchingKSetsSaveIn): (Double, Map[String, String], Double) = {
  ...
  def reIdentAttackFullData(rdstrain: DataFrame[Trace], rdstest: DataFrame[Trace], dimensions: (Int, Int, Point), in: MatMatchingKSetsSaveIn) = {
    val trainOutputMap = formSingleMatrices(rdstrain, dimensions, in.cellSize)
    val testOutputMap = formSingleMatrices(rdstest, dimensions, in.cellSize)
    reIdentFullData(trainOutputMap, testOutputMap, in.distanceType, (rdstrain.keys.toSet ++ rdstest.keys.toSet).map(trimUser(_)))
  }
  ...
  def initializePoint(in: MatMatchingKSetsSaveIn): (Point, Point) = {
    var p1 = Point(0, 0)
    var p2 = Point(0, 0)
...
case class MatMatchingKSetsSaveIn(
...
case class MatMatchingKSetsSaveOut(
