...
class HMConfusionPortionOp extends Operator[HMConfusionPortionIn, HMConfusionPortionOut] with SparkleOperator {

  override def execute(in: HMConfusionPortionIn, ctx: OpContext): HMConfusionPortionOut = {

    val dstrain = read(in.train, env)
    val dstest = read(in.test, env)

    println("---------------------train/test---------------------------")
    /*dstrain.foreach(t => println(s"user= ${t.user} id=${t.id} nbEvents=${t.events.length}"))
    println("---------------------test---------------------------")
    dstest.foreach(t => println(s"user= ${t.user} id=${t.id} nbEvents=${t.events.length}"))*/



    val (p1, p2) = (LatLng.degrees(in.lat1, in.lng1).toPoint, LatLng.degrees(in.lat2, in.lng2).toPoint)
    println("    val (p1, p2) = (LatLng.degrees(in.lat1, in.lng1).toPoint, LatLng.degrees(in.lat2, in.lng2).toPoint)")
    val (rdstrain, ratio1) = restrictArea(dstrain, p1, p2)
    println("    val (rdstrain, ratio1) = restrictArea(dstrain, p1, p2)")
    val (rdstest, ratio2) = restrictArea(dstest, p1, p2)
    println("    val (rdstest, ratio2) = restrictArea(dstest, p1, p2)")
    val dimensions = computeMatricesSize(p1, p2, in.cellSize)
    println("    val dimensions = computeMatricesSize(p1, p2, in.cellSize)")
    val train = formSingleMatrices(rdstrain, dimensions, in.cellSize)
    println("    val train = formSingleMatrices(rdstrain, dimensions, in.cellSize)")
    val test = formSingleMatricesByTraceId(rdstest, dimensions, in.cellSize)
    println("    val test = formSingleMatricesByTraceId(rdstest, dimensions, in.cellSize)")
    println(s"---------------------train len=${train.keySet}---------------------------")
    //train.foreach(p => println(s"id=${p._1} avg=${p._2.avg()} nbcells=${p._2.data.keys.size}") )
    println(s"---------------------test len=${test.keySet}---------------------------")
    //test.foreach(p => println(s"id=${p._1} avg=${p._2.avg()} nbcells=${p._2.data.keys.size}") )
    ...
    val seeds = (rdstrain.keys ++ rdstest.keys ++ rdstrain.keys.map(_.split("-")(0)) ++ rdstest.keys.map(_.split("-")(0))).map(id => (id, rnd.nextLong())).toMap
    println(seeds)
    val output = test.par.map {       case (id, mat) =>       id -> transform(id, mat, train, -51, in,ctx,in.politic)     }.seq
    println("-----------------------------------TRANSFROM DONE FOR ALL USERS-----------------------------------")
    ...
    println(s"---------------------newtest len ${newtest.keySet}---------------------------")
    //newtest.foreach(p => println(s"id=${p._1} avg=${p._2.avg()} nbcells=${p._2.data.keys.size}") )

    val protectedDataset = rdstest.map { t =>
      if (newtest.contains(t.id)){
         val matTest =  newtest(t.id)
        val seed = seeds(t.user)
        println("MODIFIED")

        createProtectedTraceAdd(t,matTest, trainEventsMap, dimensions._3, in.cellSize,seed , in.minGap , in.fminGap.getOrElse(in.minGap),in.minTime1km, in.maxDuration)
      } else {
        println("NOT MODIFIED")
        t
      }
    }
    println(s"---------------------protectedDataset len=${protectedDataset.keys}---------------------------")
    //protectedDataset.foreach(t => println(s"user= ${t.user} id=${t.id} nbEvents=${t.events.length}"))
    //val trace=protectedDataset.load("025")
   // println("loaded 025")
   // val t=trace.next()
    //println(s"${t.size}")
   // println(s"t : $t")
    //println(s"events : ${t.events}")
    //println("PRINTED TRACE")
    //HMConfusionPortionOut(Map[String,String](),0.0,0.0,0.0,Map[String,String](),null)
   // HMConfusionPortionOut(Map[String,String](),0.0,0.0,0.0,Map[String,String](),write[Trace](protectedDataset,ctx))
    HMConfusionPortionOut(statistics, newACFscore.values.sum / newACFscore.values.size, theoric_rate,matches.count( a => a._1==a._2).toDouble/matches.size, matches, write(protectedDataset, ctx.workDir))
    //HMConfusionPortionOut(statistics, newACFscore.values.sum / newACFscore.values.size, theoric_rate,matches.count( a => a._1==a._2).toDouble/matches.size, matches, write(protectedDataset, ctx.workDir))
  }



  override def isUnstable(in: HMConfusionPortionIn): Boolean = true
 ...
 private  def createProtectedTraceRandom(tr: Trace, tprime: MatrixLight[Int], up: Point, cellSize: Distance, seed: Long) = {
  ...
  private def createProtectedTraceAdd(tr: Trace, tprime: MatrixLight[Int], trainEventsMap: Map[String, Map[(Int, Int), Seq[Event]]], up: Point, cellSize: Distance ,seed: Long, dtmin: Duration,fdtmin : Duration , tvmin: Duration,maxDuration : Duration) = {
    println("CREATE PROTECTION TRACE")
    //try{
    val eventsMap = traceMatrixCutting(tr, up, cellSize)
    println(s" val eventsMap = traceMatrixCutting(tr, up, cellSize)")
    //println(s" val $eventsMap = traceMatrixCutting($tr, $up, $cellSize)")

    var gs = gaps(tr, dtmin)
    println(s"    var gs = gaps(tr, dtmin)")
   // println(s"    var gs = gaps($tr, $dtmin)")
    ...
    println(s"seeds $seedAdd $seedReinforce")

    val reinforcedCellEvents = eventsMap.flatMap {
      case p if tprime(p._1._1, p._1._2) != 0 =>  constructSetEventLighter(p._2, tprime(p._1._1, p._1._2), seedReinforce).sortBy(e => e.time)
      case _ => None
    }


    /*val reinforcedCellEvents = eventsMap.flatMap {
      case p if tprime(p._1._1, p._1._2) != 0 => {

        val a= constructSetEvent(p._2, tprime(p._1._1, p._1._2), seedReinforce)//.sortBy(e => e.time)
       // println(s"$p => ${tprime(p._1._1, p._1._2)} != 0 ")
       // println(s"set of events $a from ${eventsMap.size} event-set")
        println(s"from ${eventsMap.size} event-set")
        a
      }
      case _ => {
        println(s"none")
  ...
  //println(s"tprime = $tprime")

    //val eventsFiltered= eventsMap.filter(p =>  p._1._1 == 16817 &&  p._1._2 ==34368)
   // eventsFiltered.foreach{z => println(s"from ${eventsFiltered.size} event-set  foreach AZZZERFD")    }
  //  eventsFiltered.flatMap{z => println(s"from ${eventsFiltered.size} event-set  flatmap AZZZERFD");Set[String]()    }
/*
    val reinforcedCellEvents = eventsFiltered.flatMap {
      case p if tprime(p._1._1, p._1._2) != 0 => {
       // println(s"${p._1} from ${eventsFiltered.size} event-set bbbkkffl")
       // val a= constructSetEvent(p._2, tprime(p._1._1, p._1._2), seedReinforce)//.sortBy(e => e.time)
       val a= constructSetEventLighter(p._2, tprime(p._1._1, p._1._2), seedReinforce)//.sortBy(e => e.time)
        // println(s"$p => ${tprime(p._1._1, p._1._2)} != 0 ")
        // println(s"set of events $a from ${eventsMap.size} event-set")
        //Seq[Event]()
      ...
      case p if tprime(p._1._1, p._1._2) == 0 => {
        ...
        println(s"${p._1} from ${eventsMap.size} event-set bbbkkffl2")
        Seq[Event]()
      }
      case _ => {
        println(s"none")
    ...
    println("RE EVENTS COMPUTED")

   // println(s"RE EVENTS $reinforcedCellEvents")
    ...
    println("ADD EVENTS COMPUTED")
    //println(s"ADD EVENTS $addedEvents")

    val allEvents = (reinforcedCellEvents++addedEvents).toSeq.sortBy(e => e.time)
    println("all events")
    //println(s"ALL EVENTS $allEvents")
    //println(s"erase pois ${erasePOIs(allEvents)}")
    //println(s"allevents.size = ${allEvents.size} erasePOIs(allEvents).size = ${erasePOIs(allEvents)}")
    tr.replace(allEvents)
    //val t=Trace(tr.id, erasePOIs(allEvents))
    //println(s"t before $t")
 ...
 private def erasePOIs(events : Seq[Event]) = {
  ...
  private def eventToFit(id: String, n: Int, fgaps: Map[Int, (Event, Event)], possible: ListBuffer[Seq[Event]], dtmin: Duration, seed: Long,maxDuration : Duration, method: String = "BEST-FIT") = {
    if (fgaps.nonEmpty && possible.nonEmpty) {
      ...
      (i, constructSetEventLighter(translateEventBetweenGap(id, g, es), n, seed), "FOUND")
    } else {
  ...
  def constructSetEventLighter(in: Seq[Event], n: Int, seed: Long) = {
    val initialSet = if (in.size == 1) {
      val e = in.head
      val p = e.point
    ...
    val p = initialSet.size
    // k the limit number of time the sub-trace is sub-divided (i.e., a point in generated between each two points recuersevly k times)
    // the formula of points is U(k+1) = 2*(U(k)-1)+1 with U(0)=p  <=> U(k) = 2^n * (p-1) + 1
    // The number of sub-division we need to do respects the constraint  u(k) >= n  then  k >= log2( (n-1) / (p-1) )
    // hf the minimal k verifying the equation and h the minimal integer
    val hf = if(n>p)math.log((n-1).toDouble/(p-1))/math.log(2) else 0.0
    val h = math.ceil(hf)
    val hpow2 = math.pow(2,h).toInt
    // We will generate "limitPosition=2^h * (p-1) + 1" number of positions, we take randomly n points from those possible positions
    val limitPosition = (p-1)*math.pow(2,h).toInt+1
    println(s"id=${if(p>0)initialSet.head.user else "EMPTY" } n=$n p = $p hf = $hf h=$h limitPositon=$limitPosition  REQUIRE ${if(limitPosition>=n) "OK" else "FAILED"} ")
    require(limitPosition>=n) // sanity  check
    val taken = ListBuffer.empty[Event]
    val indices = ListBuffer.range(0,limitPosition)
    val randomGenerator = new Random(seed)

    while (taken.size < n) {
      val rnd = randomGenerator.nextInt(indices.size)
      val ind = indices(rnd)
      //println(s" ind = $ind rnd = $rnd  taken.size = ${taken.size}")
     //OLD if(((ind/p)&((ind/p)-1)) == 0){  // ind is part of the initial set if and only if it has the form of p*2^k  ( x & (x-1) == 0 verify if x is a power of two)
      if(ind % hpow2== 0){  // ind is part of the initial set if and only if it has the form of k*2^h  ( x & (x-1) == 0 verify if x is a power of two)

      //old  val k = if(ind>p) (math.log(ind/p)/math.log(2)).toInt else 0 // search for k
      val k = ind/hpow2
        //println(s"intialSet($k) = ${initialSet(k)}")
        taken += initialSet(k)
      }else{
       /* OLD val prec = if(ind > p )(math.log(ind/p)/math.log(2)).toInt else 0 // log2(ind/p) because real prec ==  2^prec * p  == ind/p
        val next = prec+1 // or log2(ind/p) + 1   because real next == 2^next * p == 2^(prec + 1)*p
        val translationRatio = ind.toDouble/prec - 1 //  (r-p2^k)/(p2^(k+1)-p2^k)
        */
        val prec = ind/hpow2  //   prec * 2^h < ind < next * 2^h
        val next = prec  +1
        val translationRatio = (ind / hpow2) / hpow2.toDouble  //  (ind - realprec) / (realnext - realprec)  =  (ind/hpow2)/hpow2.toDouble
        //println(s"prec = $prec next = $next ratio=$translationRatio")
        if(prec>=initialSet.size || next>=initialSet.size){
          println(s"OUT OF BOUND PROBLEM rnd=$rnd ind=$ind prec=$prec next=$next ratio = $translationRatio=${if(p>0)initialSet.head.user else "EMPTY" } n=$n p = $p hf = $hf h=$h limitPositon=$limitPosition  REQUIRE ${if(limitPosition>=n) "OK" else "FAILED"} ")

        }
        val e1= initialSet(prec)
        val e2= initialSet(next)

        val newP = new Point((e2.point.x - e1.point.x)*translationRatio + e1.point.x,(e2.point.y - e1.point.y)*translationRatio + e1.point.y )
        val i1 = e1.time
        val i2 = e2.time

        val newInstant = new Instant( ((i2.getMillis - i1.getMillis)*translationRatio).toLong + i1.getMillis)
        taken += Event(e1.user, newP, newInstant)
      }
      indices.remove(rnd)
    }
    //println(s"taken done ${taken.size} == $n ")
    taken
  }
  ...
  def constructSetEvent(initialSet: Seq[Event], n: Int, seed: Long) = {
    ...
    while (positions.size < n) {
      cpt=cpt+1
      //if(cpt%10==1) println(s"positions.size < n  ${positions.size} < $n")
      println(s"$cpt\t:\t positions.size < n  ${positions.size} < $n")
      positions = createDoublePosition(positions)
    }
    println(s"randomlyChoose(${positions.size}, $n, $seed) FINISH RC")
  ...
  def randomlyChoose(available: Seq[Event], n: Int, seed: Long) = {
    ...
    while (taken.size < n) {
      ...
      indices.remove(rnd)
      taken = taken :+ e
      println(s" rnd = $rnd  ind=$ind indices.size=${indices.size} taken.size=${taken.size}")
    }
    taken
  }
  ...
  private def transform(idTrace: String, mat_k: MatrixLight[Int], train: immutable.Map[String, MatrixLight[Int]], distanceType: Int, in: HMConfusionPortionIn, ctx : OpContext,politic : String) = {

    val user = idTrace.split('-')(0)
    println(s"user= $user  id=$idTrace , direct data ${ctx.workDir.toAbsolutePath.toString}")
    //new File(ctx.workDir.toAbsolutePath.toString).mkdir()

    val path = ctx.workDir.toAbsolutePath.toString+"iteration"
    new File(path).mkdirs()
    val file = new File(path+"/"+idTrace+".csv")
      ...
      while (it < ITMAX && !stop && realit < ITMAX*5) {
        ...
        // println(s"---- $dtu > $dtv  &&  $uttprime > $utbest    ---  ${tprime.indices.size}  -- ${tprime.sum} --  ${tprime.data.count(p => p._2 != 0)}")
         //println(s"$user,$realit,$dtu,$dtv,$uttprime,$utbest,${tprime.indices.size},${tprime.sum}")
          bw.write(s"$idTrace,$realit,$dtu,$dtv,$uttprime,$utbest,${tprime.indices.size},${tprime.sum}\n")
...
case class HMConfusionPortionIn(
...
case class HMConfusionPortionOut(
