...
class PoisReidentKSetSaveOp  extends Operator[ReidentificationKSetIn, ReidentificationKSetOut] with SparkleOperator {
...
*/
  ...
  private def getDistances(trainPois: Seq[PoiSet], testPois: Seq[PoiSet]) = {
    ...
    testPois.foreach { pois =>
      ...
      costs.synchronized {
        costs += trimUser(pois.user) -> distances
      }
    }
  ...
  private def getMatches(distances: Map[String, Seq[(String, Double)]], userFull : Set[String]) = {
    ...
    var probabilites : Map[String, Map[String,Double]] = Map[String, Map[String,Double]]()
     ...
     distances.foreach{ case (testUser, res) =>
       if(res.nonEmpty) {
         var level = res.map(_._1).indexOf(testUser) + 1
...
case class ReidentificationKSetSaveIn(
...
case class ReidentificationKSetSaveOut(
