...
/**
  * Implementation of a re-identification attack using heatmaps as a discriminating information. The heatmaps
  * are used to model the behaviour of training users, and then extracted from the tracks of test users and compared to
  * those from the training users. 
  * 
  */
...
@Op(
 ...
 class MatMatchingOp @Inject()(env: SparkleEnv) extends Operator[MatMatchingIn, MatMatchingOut] with SparkleOperator {
  ...
  def initializePoint(in: MatMatchingIn): (Point, Point) = {
    var p1 = Point(0, 0)
    var p2 = Point(0, 0)
  ...
  def restrictArea(ds: DataFrame[Trace], p1: Point, p2: Point): (DataFrame[Trace], Double) = {
    ...
    def reIdentAttack(rdstrain: DataFrame[Trace], rdstest: DataFrame[Trace], dimensions: (Int, Int, Point), in: MatMatchingIn): (Double, immutable.Map[String, String], Double) = {
